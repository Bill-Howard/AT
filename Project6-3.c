...
int main(void)
{
    ...
    while (1)
    {
        ...
        {
            list_o_records[num_records++] = add_record();
            printf("\n");
        }
        else if (strcmp(choice, del) == 0)
        {
            printf("Enter number: ");
            scanf("%d", &rec_no);
            printf("%s was deleted.\n\n", list_o_records[rec_no - 1].name);
            memset(list_o_records[rec_no - 1].name, 0, sizeof(list_o_records[rec_no - 1].name));
            memset(list_o_records[rec_no - 1].email, 0, sizeof(list_o_records[rec_no - 1].email));
            memset(list_o_records[rec_no - 1].phone, 0, sizeof(list_o_records[rec_no - 1].phone));
            num_records--;
        }
